...
from experiments.harness import build_truth, compute_metrics, fmt, load_universe, random_splits, run_perdrug
...
def blend(a, b, w=0.5):
    ...
    for d in set(a)|set(b):
        da=a.get(d,{}); db=b.get(d,{})
        keys=set(da)&set(db)
        if keys: out[d]={i: w*da[i]+(1-w)*db[i] for i in keys}
    return out
...
best=dict(max_leaf_nodes=127,learning_rate=0.05,max_iter=600,l2_regularization=1.0)
