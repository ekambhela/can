...
- `IC50_gdsc2.csv.gz` — natural-log IC50 from the GDSC2 assay for the 806 GDSC2 cell lines that are also in `genomic_features_v17` (`COSMIC_ID` + `Drug_<id>_IC50`). Drug ids are namespaced `200000 + GDSC2_drug_id` so they never collide with GDSC1 ids. The file holds the 126 GDSC2 compounds not obviously in the GDSC1 panel; `model/gdsc.py` then drops any whose name still normalizes to a GDSC1 drug, leaving **121 GDSC2 compounds modeled** (248 GDSC1 + 121 GDSC2 = 369 total), so no drug appears twice.
